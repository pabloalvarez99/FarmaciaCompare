...
import { derivedHref, type ScrapedProductResult } from '@/lib/api-products';
 ...
 *
 * The link follows the evidence, strongest first: barcode is an exact EAN
 * identity, medicationId is a matcher-inferred catalog link, and
 * derivedGroupKey is the product's own description — the only one a shampoo or
 * a lipstick usually has, since those categories publish neither.
 */
export function ProductCard({ product }: { product: ScrapedProductResult }) {
  const barcode = product.barcode?.trim();
  const medicationId = product.medicationId?.trim();
  const derivedKey = product.derivedGroupKey?.trim();
  const href = barcode
    ? `/precios/${encodeURIComponent(barcode)}`
    : medicationId
      ? `/precios/medicamento/${encodeURIComponent(medicationId)}`
      : derivedKey
        ? derivedHref(derivedKey)
        : null;
    ...
    <div className="flex gap-3 p-3 sm:gap-4 sm:p-4">
      ...
      <div className="flex min-w-0 flex-1 flex-col gap-2 sm:flex-row sm:items-start sm:justify-between sm:gap-6">
        <div className="min-w-0">
          <p className="font-medium leading-snug text-foreground">{product.name}</p>
          {lab && <p className="mt-0.5 truncate text-sm text-muted-foreground">{lab}</p>}
          <div className="mt-2 flex flex-wrap items-center gap-1.5">
            <PharmacyChip name={product.pharmacy.name} />
  ...
  if (!href) {
    return <li className="bg-card">{body}</li>;
  }
