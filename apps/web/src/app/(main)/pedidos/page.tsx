    ...
    <div className="max-w-3xl mx-auto px-4 py-8">
      ...
      {orders.length === 0 ? (
        <Card className="p-8 text-center">
          <p className="text-gray-500 mb-2">No tienes pedidos todavía</p>
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    ...
                    {order.items?.slice(0, 2).map((item) => (
                      <p key={item.id} className="text-xs text-gray-400 mt-0.5 truncate">
                  ...
                  <div className="text-right shrink-0">
                    <p className="text-lg font-bold text-gray-900">{formatCLP(order.totalAmount)}</p>
                    <p className="text-xs text-gray-400 mt-1">Pedido #{order.id.slice(0, 8)}</p>
                  </div>
                </div>
              </Card>
            );
